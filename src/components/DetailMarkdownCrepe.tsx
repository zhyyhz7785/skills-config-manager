...
import { ccmPerfSpan } from '../lib/ccmPerf'
...
/** 同一次挂载内 size 失败自动重试上限；超限停在失败横幅，避免建-毁循环闪烁 */
const SIZE_RETRY_MAX = 3
const COPY_FLASH_MS = 1200

function failLabel(reason: CreateFailReason): string {
  if (reason === 'size') return t('md.sizeFail')
  if (reason === 'timeout') return t('md.createTimeout')
  return t('md.createFail')
}
...
function splitPath(filePath: string): { fileName: string; dir: string } {
  ...
  if (!normalized) return { fileName: t('md.noFile'), dir: '' }
...
export function DetailMarkdownCrepe({
  ...
  const [sizeRetryCount, setSizeRetryCount] = useState(0)
  const [copyFlash, setCopyFlash] = useState<'full' | 'plain' | null>(null)
  ...
  const { fileName, dir } = useMemo(() => splitPath(filePath), [filePath, locale])
  const fallbackBody = useMemo(() => {
    const { body } = splitFrontmatter(initialFullTextRef.current)
    return body || initialFullTextRef.current || t('md.noContent')
  }, [locale, t])
  ...
  const reportFail = useCallback((reason: CreateFailReason, detail?: unknown) => {
    console.error(`[DetailMarkdownCrepe] create ${reason}:`, detail ?? failLabel(reason))
    const msg = formatFailDetail(detail)
    createFailRef.current = reason
    if (reason === 'size') {
      sizeRetryRef.current += 1
      setSizeRetryCount(sizeRetryRef.current)
    }
  ...
  const ensureCreated = useCallback(
    async (opts?: { force?: boolean }) => {
      ...
      if (!opts?.force && fail === 'size' && sizeRetryRef.current >= SIZE_RETRY_MAX) return
  ...
  useEffect(() => {
    ...
    const kick = () => {
      ...
      raf = requestAnimationFrame(() => {
        ...
        if (fail === 'size' && sizeRetryRef.current >= SIZE_RETRY_MAX) return
        // size：仅在尺寸恢复边沿重试；无失败时也可首次创建
        if (fail === 'size' && !grew && createInflightRef.current) return
        if (fail === 'size' && !grew) {
          // 已有尺寸且仍是 size（例如 create 后测高为 0 又回到有尺寸）：允许再试，受 SIZE_RETRY_MAX 约束
  ...
  const failBannerText =
    createFail && (createFail !== 'size' || sizeRetryCount >= SIZE_RETRY_MAX)
      ? createFail === 'size'
        ? failLabel('size')
        : createFailDetail
          ? `${failLabel(createFail)}：${createFailDetail}`
          : failLabel(createFail)
      : null

  const flashCopy = useCallback((which: 'full' | 'plain') => {
    setCopyFlash(which)
  ...
  const copyFullMarkdown = useCallback(() => {
    let text = initialFullTextRef.current
    if (editableRef.current && crepeRef.current && readyRef.current) {
      try {
        const live = crepeRef.current.getMarkdown()
        const normalized = live === '\u200b' || live.trim() === '\u200b' ? '' : live
        text = joinFrontmatter(fmRef.current, normalized)
      } catch {
        // 序列化失败则退回打开时的原文
      }
    }
  ...
  const copyPlainText = useCallback(() => {
    const live = rootRef.current?.innerText?.trim()
    const text = live || initialFullTextRef.current
    void navigator.clipboard.writeText(text).then(
      () => flashCopy('plain'),
      () => undefined,
    )
    ...
    <div className="detail-md-crepe" ref={wrapRef}>
      <div className="detail-md-docbar" title={filePath || undefined}>
        ...
        <div className="detail-md-docbar-actions">
          {!editable ? <span className="detail-md-readonly">{t('md.readonly')}</span> : null}
          <button
            type="button"
            className="detail-md-copy"
            title={t('md.copyFullTitle')}
            onClick={copyFullMarkdown}
          >
            {copyFlash === 'full' ? t('md.copied') : t('md.copyFull')}
          </button>
          <button
            type="button"
            className="detail-md-copy"
            title={t('md.copyPlainTitle')}
            onClick={copyPlainText}
          >
            {copyFlash === 'plain' ? t('md.copied') : t('md.copyPlain')}
          </button>
          {editable ? (
            <button
              type="button"
              className="detail-md-save"
              title={t('md.saveShortcut')}
              onClick={() => void save()}
            >
              {t('md.save')}
            </button>
      ...
      <div className="detail-md-editor-slot">
        {!editorReady ? (
          <pre className="detail-md-fallback" aria-label={t('md.fallback')}>
            {fallbackBody}
          </pre>
        ) : null}
