...
export function sideLabel(pathSide: MdPathSide): string {
  return pathSide === 'container' ? '容器' : '库'
}
...
type DetailMarkdownTabHostProps = {
  ...
  onOpenCompare?: (entryId: string) => void
...
export function DetailMarkdownTabHost({
  ...
  if (tabs.length === 0) {
    return <div className="detail-md-empty">选择左侧条目查看 Markdown</div>
  }

  const activeTab = tabs.find((t) => t.tabId === activeId)
  const canCompareActive =
    Boolean(onOpenCompare) &&
    activeTab?.kind === 'edit' &&
    Boolean(activeTab.entryId)

  return (
    <div className="detail-md-tab-host">
      <div className="detail-md-tabs" role="tablist" aria-label="已打开的 Markdown">
        {tabs.map((tab) => {
        ...
        {canCompareActive ? (
          <button
            type="button"
            className="detail-md-tab-compare"
            title="对比代码（永久库）与容器"
            onClick={() => onOpenCompare?.(activeTab!.entryId)}
          >
            对比
          </button>
        ) : null}
          ...
          return (
            ...
            >
              {tab.kind === 'diff' ? (
                ...
                <Suspense
                  fallback={<div className="detail-md-empty">加载编辑器…</div>}
                >
                  <DetailMarkdownCrepe
