...
export function isWhitespaceOnlyDiff(leftText: string, rightText: string): boolean {
  ...
  const norm = (s: string) =>
    s
      ...
      .join('\n')
  return norm(leftText) === norm(rightText)
}
...
function eolLabel(style: EolStyle): string {
  switch (style) {
    case 'lf':
      return 'LF'
    case 'crlf':
...
/**
 * 比对横幅文案：纯行尾差异优先说明左右行尾类型；否则回落空白提示；无差异返回 null。
 */
export function whitespaceDiffBanner(leftText: string, rightText: string): string | null {
  if (leftText === rightText) return null
  const leftEol = detectEolStyle(leftText)
  const rightEol = detectEolStyle(rightText)
  const bodySame =
    leftText.replace(/\r\n/g, '\n').replace(/\r/g, '\n') ===
    rightText.replace(/\r\n/g, '\n').replace(/\r/g, '\n')
  if (bodySame && leftEol !== rightEol) {
    return `两侧正文相同，仅行尾符不同（左 ${eolLabel(leftEol)} · 右 ${eolLabel(rightEol)}）`
  }
  if (isWhitespaceOnlyDiff(leftText, rightText)) {
    return '两侧可见正文相同，差异主要为空白或行尾空格'
  }
  return null
}
