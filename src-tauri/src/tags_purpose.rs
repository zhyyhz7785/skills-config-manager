...
use crate::catalog::{
    get_entry_tags, kind_label, list_projects, load_catalog, update_entry_level, update_entry_tags,
...
pub fn set_entry_level(
    ...
    for id in &ids {
        update_entry_level(lib, id, level)?;
    }
    let label = if clear { "未分类" } else { upper.as_str() };
    Ok(TagsOpResult {
        ok: true,
        message: format!("已设为 {label}（{} 项）", ids.len()),
        data: None,
        snapshot: snap(settings),
    })
}
...
pub fn apply_suggested_purposes(
    ...
    for item in items.into_iter().filter(|i| i.selected) {
        if let Some(ref lv) = item.suggested_level {
            ...
            if u == "L0" || u == "L1" || u == "L2" {
                update_entry_level(lib, &item.entry_id, Some(&u))?;
                level_n += 1;
            }
        }
        ...
        if !purpose.is_empty() {
            ...
            update_entry_tags(lib, &item.entry_id, &scope, &purposes)?;
            purpose_n += 1;
        }
    }
    Ok(TagsOpResult {
...
mod tests {
    ...
    fn set_level_and_tags_roundtrip() {
        ...
        let load = load_catalog(&lib);
        let tags = get_entry_tags(load.catalog.entries.iter().find(|e| e.id == "s1").unwrap());
