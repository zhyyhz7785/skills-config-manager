...
pub fn create_project_container(
    ...
    if let Some(mut ex) = existing {
        ...
        return Ok(ProjectOpResult {
            ok: true,
            message: format!("容器目录已就绪：{cursor_s}（已选中并置于置顶「{}」）", ex.name),
...
pub fn reorder_project_cmd(
    ...
    let lib = require_library(settings)?;
    reorder_project(lib, &id, direction.trim())?;
    Ok(ProjectOpResult {
        ok: true,
        message: "顺序已更新".into(),
        data: None,
...
pub fn open_project_cursor(settings: &AppSettings, id: String) -> Result<ProjectOpResult, String> {
    let lib = require_library(settings)?;
    let project = find_project(lib, &id)?.ok_or_else(|| "项目不存在".to_string())?;
    let cursor = Path::new(&project.root_path).join(".cursor");
...
pub fn open_project_root(settings: &AppSettings, id: String) -> Result<ProjectOpResult, String> {
    let lib = require_library(settings)?;
    let project = find_project(lib, &id)?.ok_or_else(|| "项目不存在".to_string())?;
    open_path(project.root_path)?;
    Ok(ProjectOpResult {
        ok: true,
        message: "已打开项目根".into(),
        data: None,
...
mod tests {
    ...
    fn create_then_edit_pin_reorder_remove() {
        ...
        toggle_pin_project(&settings, id.clone()).unwrap(); // pin again
        let _ = reorder_project_cmd(&settings, id2.clone(), "up".into());
