...
use crate::catalog::{save_catalog, LibraryCatalog};
...
/// Copy current catalog.json to a user-chosen path.
pub fn export_catalog_to_path(library_root: &str, dest: &str) -> Result<(), String> {
    let lib = library_root.trim();
    if lib.is_empty() {
        return Err("永久库根为空".into());
    }
    let dest = dest.trim();
    if dest.is_empty() {
        return Err("存储路径为空".into());
    }
    let src = catalog_path(lib);
    if !src.is_file() {
        return Err("当前没有 catalog.json，无法存储".into());
    }
    if let Some(parent) = Path::new(dest).parent() {
        if !parent.as_os_str().is_empty() {
            fs::create_dir_all(parent).map_err(|e| format!("mkdir 存储目录: {e}"))?;
        }
    }
    fs::copy(&src, dest).map_err(|e| format!("存储台账失败: {e}"))?;
    Ok(())
}

/// Validate and overwrite catalog.json from a user-chosen file.
/// Writes `{library}/catalog.json.bak` first when a live catalog exists.
pub fn import_catalog_from_path(library_root: &str, src: &str) -> Result<(), String> {
    let lib = library_root.trim();
    if lib.is_empty() {
        return Err("永久库根为空".into());
    }
    let src = src.trim();
    if src.is_empty() {
        return Err("读取路径为空".into());
    }
    let src_path = Path::new(src);
    if !src_path.is_file() {
        return Err(format!("文件不存在: {src}"));
    }
    let raw = fs::read_to_string(src_path).map_err(|e| format!("读台账失败: {e}"))?;
    let catalog: LibraryCatalog =
        serde_json::from_str(&raw).map_err(|e| format!("台账 JSON 无效: {e}"))?;

    let live = catalog_path(lib);
    if live.is_file() {
        let bak = Path::new(lib).join("catalog.json.bak");
        fs::copy(&live, &bak).map_err(|e| format!("写 catalog.json.bak 失败: {e}"))?;
    }
    save_catalog(lib, &catalog)?;
    Ok(())
}
...
mod tests {
    ...
    fn summarize_includes_kind_and_samples() {
        ...
        c.entries.push(CatalogEntry {
            id: "rule-b".into(),
            kind: "rule".into(),
            library_path: "rules/b/b.mdc".into(),
            is_in_library: true,
            ..Default::default()
        });
    ...
    #[test]
    fn export_then_import_replaces_entries() {
        let dir = tempfile::tempdir().unwrap();
        let lib = dir.path().to_string_lossy().to_string();
        ensure_library_layout(&lib).unwrap();
        ...
        let dest = dir.path().join("out").join("catalog-copy.json");
        export_catalog_to_path(&lib, &dest.to_string_lossy()).unwrap();
        assert!(dest.is_file());

        save_catalog(&lib, &crate::catalog::empty_catalog()).unwrap();
        assert!(load_catalog(&lib).catalog.entries.is_empty());

        import_catalog_from_path(&lib, &dest.to_string_lossy()).unwrap();
        let load = load_catalog(&lib);
        assert_eq!(load.catalog.entries.len(), 1);
        assert_eq!(load.catalog.entries[0].id, "keep-me");
        assert!(Path::new(&lib).join("catalog.json.bak").is_file());
    }
    ...
    fn import_rejects_invalid_json() {
        ...
        let err = import_catalog_from_path(&lib, &bad.to_string_lossy()).unwrap_err();
