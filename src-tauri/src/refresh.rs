...
use crate::catalog::{
    list_projects, load_catalog, upsert_entry, validate_entry_paths, CatalogEntry, CatalogOrigin,
};
...
fn collect_live_dual_copy_conflicts(
    ...
    for e in &load.catalog.entries {
        ...
        if src_hash.eq_ignore_ascii_case(&tgt_hash) {
            continue;
        }
...
/// Lightweight reconcile: relink deployedPath + soft-register new container files.
fn apply_reconcile_mvp(settings: &AppSettings, discovered: &[DiscoveredItemDto]) -> Result<(), String> {
    ...
    let mut by_id: HashMap<String, CatalogEntry> = load
        .catalog
        .entries
        .iter()
        .cloned()
    ...
    for item in discovered {
        ...
        let Some(entry) = by_id.get_mut(&item.suggested_id) else {
            continue;
        };
        if entry.library_path.trim().is_empty() {
            continue;
    ...
    // Soft-register: container file not in catalog → copy + upsert
    let known_ids: HashSet<String> = by_id.keys().cloned().collect();
    for item in discovered {
        if under_dir(&item.source_path, lib) {
            continue;
        }
        ...
        if !dest.exists() {
            if let Err(e) = copy_path(source, &dest) {
                eprintln!("refresh soft-register copy: {e}");
                continue;
            }
        }
